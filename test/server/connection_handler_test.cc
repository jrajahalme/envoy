...
public:
  ...
  public:
    TestListener(ConnectionHandlerTest& parent, uint64_t tag, bool bind_to_port,
                 bool hand_off_restored_destination_connections, bool transparent,
                 const std::string& name)
        : parent_(parent), tag_(tag), bind_to_port_(bind_to_port),
          hand_off_restored_destination_connections_(hand_off_restored_destination_connections),
          transparent_(transparent), name_(name) {}
  ...
  TestListener* addListener(uint64_t tag, bool bind_to_port,
    ...
    TestListener* listener = new TestListener(
        *this, tag, bind_to_port, hand_off_restored_destination_connections, transparent, name);
...
TEST_F(ConnectionHandlerTest, RemoveListener) {
  ...
  EXPECT_CALL(dispatcher_, createListener_(_, _, _, false, _))
      .WillOnce(Invoke([&](Network::Socket&, Network::ListenerCallbacks& cb, bool, bool,
                           bool) -> Network::Listener* {
        listener_callbacks = &cb;
        return listener;
      }));
  TestListener* test_listener = addListener(1, true, false, false, "test_listener");
...
TEST_F(ConnectionHandlerTest, DisableListener) {
  ...
  TestListener* test_listener = addListener(1, false, false, false, "test_listener");
  EXPECT_CALL(test_listener->socket_, localAddress());
  handler_->addListener(*test_listener);
...
TEST_F(ConnectionHandlerTest, FindListenerByAddress) {
  TestListener* test_listener1 = addListener(1, true, true, false, "test_listener1");
  Network::Address::InstanceConstSharedPtr alt_address(
      new Network::Address::Ipv4Instance("127.0.0.1", 10001));

  Network::MockListener* listener = new Network::MockListener();
  EXPECT_CALL(dispatcher_, createListener_(_, _, _, true, _))
      .WillOnce(Invoke([&](Network::Socket&, Network::ListenerCallbacks&, bool, bool,
  ...
  TestListener* test_listener2 = addListener(2, true, false, false, "test_listener2");
...
TEST_F(ConnectionHandlerTest, NormalRedirect) {
  ...
  TestListener* test_listener2 = addListener(1, false, false, false, "test_listener2");
  Network::MockListener* listener2 = new Network::MockListener();
